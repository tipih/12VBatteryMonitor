...
public:
    static void disconnect(bool wifioff, bool eraseap) {
        mockCalls.push_back({"WiFi.disconnect", (uint64_t)((wifioff ? 1 : 0) | (eraseap ? 2 : 0))});
    }
    
    static void mode(int m) {
        mockCalls.push_back({"WiFi.mode", (uint64_t)m});
    }
};
...
public:
    ...
    static float usToMinutes(uint64_t us) {
        return us / 60000000.0f;
    }
...
void test_sleep_manager_enter_deep_sleep(void) {
    ...
    TEST_ASSERT_EQUAL_STRING("esp_sleep_enable_timer_wakeup", mockCalls[4].action.c_str());
