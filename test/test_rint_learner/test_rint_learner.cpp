...
private:
    ...
    static float compTo25C(float R_mOhm, float tempC) {
        ...
        if (!isfinite(tempC)) return R_mOhm; // No compensation if temp is invalid
...
void test_rint_learner_set_baseline(void) {
    ...
    TEST_ASSERT_EQUAL_FLOAT(40.0f, learner.baseline_mOhm()); // Should remain unchanged
...
void test_temperature_compensation(void) {
    ...
    TEST_ASSERT_EQUAL_FLOAT(35.0f, learner.lastRint_mOhm());
    TEST_ASSERT_FLOAT_WITHIN(0.5f, 33.4f, learner.lastRint25_mOhm()); // Approx 35/(1+0.005*10)
    
    // At lower temperature, Rint25 should be higher than measured Rint
    learner.setMeasuredRint(35.0f, 15.0f);
    TEST_ASSERT_EQUAL_FLOAT(35.0f, learner.lastRint_mOhm());
    TEST_ASSERT_FLOAT_WITHIN(0.5f, 36.8f, learner.lastRint25_mOhm()); // Approx 35/(1-0.005*10)
}
...
void test_soh_calculation(void) {
    ...
    TEST_ASSERT_EQUAL_FLOAT(1.0f, learner.currentSOH()); // Should be capped at 1.0
}
