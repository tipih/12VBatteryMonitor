...
public:
    ...
    bool isKey(const char* key) {
        if (storage.find(currentNamespace) == storage.end()) return false;
        return storage[currentNamespace].find(key) != storage[currentNamespace].end();
    }
...
void BatteryConfig::setCapacity(float ah) {
    ...
    if (hasP) rb = prefsR.getFloat(primaryKey, NAN);
    else if (hasF) rb = prefsR.getFloat(fallbackKey, NAN);
    prefsR.end();

    char msg[128];
    if (ok && (hasP || hasF) && isfinite(rb)) {
        snprintf(msg, sizeof(msg), "NVS: put=1 fb=%d P=%d F=%d CAP=%.3f RB=%.3f",
            usedFallback ? 1 : 0, hasP ? 1 : 0, hasF ? 1 : 0, batteryCapacityAh, rb);
    }
    else {
        snprintf(msg, sizeof(msg), "NVS: put=0 fb=%d P=%d F=%d CAP=%.3f RB=NONE",
            usedFallback ? 1 : 0, hasP ? 1 : 0, hasF ? 1 : 0, batteryCapacityAh);
    }
...
void test_battery_config_set_capacity_valid(void) {
    ...
    TEST_ASSERT_TRUE(last_ble_notification.find("CAP=15.500") != std::string::npos);
}
