...
public:
    void begin(float initialBaseline_mOhm, DebugPublisher* dbg = nullptr) {
        ...
        if (!isfinite(_baseline_mOhm) || _baseline_mOhm <= 0.5f || _baseline_mOhm > 500.0f) {
...
private:
    ...
    void push(const Sample& s) {
        ...
        if (_count < RB_CAPACITY) _count++;
    }

    bool getBack(int backIdx, Sample& out) const {
        if (backIdx < 0 || backIdx >= _count) return false;
        int idx = _head - 1 - backIdx;
        if (idx < 0) idx += RB_CAPACITY;
        out = _rb[idx];
        return true;
    }
    ...
    bool windowByTime(int anchorBackIdx, uint32_t ms, bool pre, int& outIdx) {
        Sample anchor;
        if (!getBack(anchorBackIdx, anchor)) return false;
        if (pre) {
            int b = anchorBackIdx;
            Sample s;
            while (b < _count) {
                if (!getBack(b, s)) break;
                uint32_t dt = anchor.t - s.t;
                if (dt >= ms) { outIdx = b; return true; }
                ++b;
            }
    ...
    bool findStepWindows(int& preStart, int& preEnd, int& postStart, int& postEnd) {
        ...
        for (int back = 3; back < _count; ++back) {
            Sample sPost;
            if (!getBack(back, sPost)) break;
            if (tNow - sPost.t > SCAN_BACK_MS) break;
            int backEarlier = back + 10;
            Sample sPre;
            for (int b2 = back + 1; b2 < _count; ++b2) {
                if (!getBack(b2, sPre)) break;
                uint32_t dt = sPost.t - sPre.t;
                if (dt >= STEP_SEPARATION_MS) { backEarlier = b2; break; }
            }
            if (!getBack(backEarlier, sPre)) continue;
            float dI = sPost.I - sPre.I;
            if (fabsf(dI) < I_STEP_MIN_A) continue;
            preEnd = backEarlier;
            postStart = back;
            if (!windowByTime(preEnd, STEP_WINDOW_MS, true, preStart)) continue;
            if (!windowByTime(postStart, STEP_WINDOW_MS, false, postEnd)) continue;
            if (_dbg && _dbg->ok()) {
                char js[256];
    ...
    float medianRecentR25() const {
        if (_recentCount <= 0) return _lastRint25_mOhm;
        float tmp[MED_WINDOW];
        int n = _recentCount;
        for (int i = 0; i < n; i++) tmp[i] = _recentR25[i];
        for (int i = 1; i < n; i++) {
            float key = tmp[i];
            int j = i - 1;
            while (j >= 0 && tmp[j] > key) {
    ...
    void tryDetectAndLearn(uint32_t nowMs) {
        ...
        if (fabsf(post.i) < I_ABS_MIN_A && fabsf(pre.i) < I_ABS_MIN_A) { debugReject("abs_I_low", post.i, pre.i); return; }
        if (fabsf(dI) > MAX_EVENT_dI_A) { debugReject("di_too_big", dI, MAX_EVENT_dI_A); return; }
        if (fabsf(dV) > MAX_EVENT_dV_V) { debugReject("dv_too_big", dV, MAX_EVENT_dV_V); return; }
        if (fabsf(dI) < 1e-6f) { debugReject("di_zero", dI, 0); return; }
        float R_mOhm = fabsf((dV / dI) * 1000.0f);
        if (!isfinite(R_mOhm) || R_mOhm <= 0.1f || R_mOhm > 1000.0f) { debugReject("r_out_of_range", R_mOhm, 0); return; }
    ...
    void maybeUpdateBaseline(float candidate_mOhm, uint32_t nowMs) {
        if (nowMs - _lastBaselineUpdateMs < MIN_UPDATE_INTERVAL_MS) return;
        float delta = candidate_mOhm - _baseline_mOhm;
        if (fabsf(delta) < BASELINE_DEADBAND_mOHM) return;
        ...
        if (hours <= 0.0f) hours = MIN_UPDATE_INTERVAL_MS / 3600000.0f;
        ...
        if (proposed > _baseline_mOhm) capped = fminf(proposed, _baseline_mOhm + riseCap);
        else capped = fmaxf(proposed, _baseline_mOhm - fallCap);
        if (_dbg && _dbg->ok()) {
            char js[320];
            snprintf(js, sizeof(js),
